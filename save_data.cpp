...
void UI_Mainwindow::save_wave_inspector_buffer_to_raw(struct device_settings *d_parms, short *wbuf[MAX_CHNS])
{
  ...
  for (i=0; i<MAX_CHNS; i++) {
    ...
    //associate proper waveform data depending on wbuf argument
    if (wbuf)
      wavebuf[i] = wbuf[i];
    else
      wavebuf[i] = d_parms->wavebuf[i];
  ...
  opath[0] = 0;
  if (recent_savedir[0]!=0) {
    strlcpy(opath, recent_savedir, sizeof(opath));
    strlcat(opath, "/", sizeof(opath));
  }
  strlcat(opath, "waveform.raw", sizeof(opath));

  //asking for filename
  strlcpy(opath, QFileDialog::getSaveFileName(this, "Save file", opath, "RAW files (*.raw *.RAW)").toLocal8Bit().data(), sizeof(opath));
  if (!strcmp(opath, ""))
    return;
